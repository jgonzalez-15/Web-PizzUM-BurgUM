...
public class FavoritoMapper {
    ...
    public Favorito toEntity(FavoritoRequestDTO dto) {
        ...
        favorito.setCliente(clienteRepository.findByEmail(dto.getIdCliente()).orElseThrow(()->new ClienteNoExisteException()));
        favorito.setNombre(dto.getNombre());
        return favorito;
    }

    public FavoritoResponseDTO toResponseDTO(Favorito favorito) {
        return new FavoritoResponseDTO(favorito.getId(), favorito.getNombre(),favorito.getCliente().getEmail(), favorito.getCreacion().getId());
    }
