...
export default function NewOrder() {
    ...
    useEffect(() => {
        const fetchBebidas = async () => {
            try {
                const res = await fetch("http://localhost:8080/api/producto/listarBebidas", {
                    credentials: "include",
                });
                if (!res.ok) throw new Error("Error al cargar bebidas");
                const data = await res.json();
                setBebidasDisponibles(data);
            } catch (err) {
                console.error("No se pudieron cargar las bebidas:", err);
            ...
            <div className="flex flex-col pt-20 w-screen max-w-full min-h-[calc(100vh)] justify-between bg-gray-50">
                <div className="w-full flex flex-1 flex-col md:flex-row h-full items-start justify-between gap-8 px-6 md:px-16">
                    <div className="w-full md:w-2/3 flex flex-col items-center md:items-start">
                        <h1 className="text-3xl font-extrabold text-gray-800 mb-6 mt-2">Tu carrito</h1>

                        {items.length === 0 ? (
                            <div className="flex flex-col items-center justify-center text-center bg-white shadow-md rounded-2xl p-8 w-full md:w-2/3">
                                <h2 className="text-gray-600 mb-4">No hay artículos en tu carrito todavía.</h2>
                                        ...
                                        <div className="flex flex-col gap-1 w-full">
                                            ...
                                            {item.tipo && (
                                                <p className="text-sm text-gray-500">
                                                    Tipo: <span className="text-gray-700">{item.tipo}</span>
                                                </p>
                                            )}
                            ...
                            {item.ingredientes.map((i) => i.nombre).join(", ")}
                                            ...
                                            <p className="text-base font-semibold text-orange-600 mt-1">${item.precio}</p>
                                        </div>
                                ...
                                <div className="mt-6 bg-white p-6 rounded-2xl shadow-md border border-gray-100 flex flex-col gap-4">
                                    <h3 className="font-semibold text-lg text-gray-800">Agregar bebida</h3>
                                    <p className="text-gray-600 text-sm">Elegí una bebida para acompañar tu pedido:</p>

                                    {loading ? (
                                        <p className="text-gray-500 text-sm">Cargando bebidas...</p>
                                    ) : bebidasDisponibles.length === 0 ? (
                                        <p className="text-gray-500 text-sm">No hay bebidas disponibles.</p>
                                    ) : (
                    ...
                    <div className="w-full md:w-1/3 bg-white shadow-xl rounded-2xl p-6 border border-gray-100 sticky top-24 self-start">
                        <h1 className="font-bold text-2xl mb-4 text-gray-800">Resumen de pedido</h1>

                        {items.length === 0 ? (
                            <p className="text-gray-500 text-center mt-10">No hay artículos en el carrito</p>
                        ) : (
                            <>
                                <div className="flex flex-col">
                                    {items.map((item) => (
                                        <div key={item.id || item.idProducto} className="flex justify-between py-1">
                                            <span className="text-gray-700 font-medium">{item.nombre}</span>
                                            <span className="text-gray-700 font-medium">${item.precio}</span>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="flex flex-col md:flex-row justify-center items-center gap-4 mt-6">
                                    ...
                                    <SmallButton text="Pasar al pago" isPrimary={true} route="/checkout" />
                                </div>
                            </>
                        )}
                    </div>
